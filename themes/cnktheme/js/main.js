...
var ACTION = {
    ...
    'listAction':function(e){
        ...
        var fetchUrl = SITE_URL + '/'+ type +'/';

        switch(action){
            case 'delete':
                fetchUrl += "delete/";
                var count = $(".count").text();
                var rowId = type+"-"+post_id;
                $("#" + rowId).css({"background": '#FFEAEA'});

                var t = UTILS.confirmDelete(type,name);
                if (!t){
                    $("#" + rowId).css({"background": 'none'});
                    return false;
                }

                //check the type and put a variation based on type
                if(type == 'content'){
                    var gallery = $(obj).attr('data-gallery');
                    var dataLoad = {id: post_id, gallery: gallery};
                } else {
                    var dataLoad = {id: post_id };
                }



                //do the query to remove category
                $.post(fetchUrl, dataLoad,
                    function (data) {
            ...
            case 'change-status':
                ...
                fetchUrl += "updateStatus/";
                var status = $(obj).attr('data-value');
                var gallery = $(obj).attr('data-gallery');

                $.post(fetchUrl, {'id': post_id, 'data': status, 'gallery': gallery }, function (data) {
    ...
    'ugcAction': function(e) {
        ...
        $.post(fetchUrl, {'id': post_id, 'data': action, 'gallery': gallery, 'is_ugc':1, 'type': cType }, function(data) {
                if (data.status == 'success') {
                    var btnId ="ugc-"+action+'-'+post_id;
                    var hideSpan = "btn-set-"+post_id;
                    $('#'+hideSpan).hide();
                    $('#'+btnId).show();
                } else {
                    alert(data.message);
    ...
    'shortlistAction': function(e) {
        ...
        var type = $(obj).attr('data-type');
        var rowId = 'user-'+post_id;

        var fetchUrl = SITE_URL + '/' + type + '/';

        fetchUrl += "shortlist/";

        $.post(fetchUrl, {'id': post_id, 'data': value }, function(data) {
                if (data.status == 'success') {

                    //remove the div from the listing same as doing a delete
                    $("#" + rowId).animate({opacity: 1.0}, 500).fadeOut('slow');
                    //$(".count").text( parseInt(count)-1 );
                    //var btnId ="ugc-"+action+'-'+post_id;
                    //var hideSpan = "btn-set-"+post_id;
