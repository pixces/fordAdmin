...
class Hybrid_Providers_Twitter extends Hybrid_Provider_Model_OAuth1
{
	...
	*/
    ...
    function getAppAccessToken()
    {
        ...
        $secret = $this->config['key']['secret'];
        $endpoint = "https://api.twitter.com/oauth2/token?".$key."&secret";

        $tokens = $this->api->requestToken( $endpoint );
	...
	*/ 
    ...
    function getSocialPosts($params){
        $response  = $this->api->get( 'search/tweets.json', $params );
...
//        print_r($response);      exit;
        ...
        foreach( $response->statuses as $item ){
            ...
            $sa->id                             = (property_exists($item,'id_str'))?$item->id_str:$item->id;
            $sa->date                           = (property_exists($item,'created_at'))?$item->created_at:"";
            $sa->timestamp                      = (property_exists($item,'created_at'))?strtotime($item->created_at):"";
            $sa->text                           = (property_exists($item,'text'))?$item->text:"";
            $sa->lang                           = (property_exists($item,'lang'))?$item->lang:"";
            $sa->source                         = "twitter";    // (property_exists($item,'source'))?$item->source:"";
            $sa->type                           = $this->getPostType($item);

            $name                               = (property_exists($item->user,'name'))?$item->user->name:"";
            $fullName                           = explode(" ",$name);

            $sa->user->profile->identifier      = (property_exists($item->user,'id'))?$item->user->id:"";
            $sa->user->profile->firstName       = isset($fullName[0])?$fullName[0]:"";
            $sa->user->profile->lastName        = isset($fullName[1])?$fullName[1]:"";
            $sa->user->profile->displayName     = (property_exists($item->user,'screen_name'))?$item->user->screen_name:"";
            $sa->user->profile->profileURL      = (property_exists($item->user,'screen_name'))?("http://twitter.com/".$item->user->screen_name):"";
            $sa->user->profile->photoURL        = (property_exists($item->user,'profile_image_url'))?$item->user->profile_image_url:"";
            $sa->user->profile->language       = (property_exists($item->user,'profile_image_url'))?$item->user->profile_image_url:"";
            $sa->user->profile->region         = (property_exists($item->user,'location'))?$item->user->location:"";
            $sa->user->profile->followersCount = (property_exists($item->user,'followers_count'))?$item->user->followers_count:"";
            $sa->user->profile->friendsCount   = (property_exists($item->user,'friends_count'))?$item->user->friends_count:"";
            $sa->user->profile->statusCount    = (property_exists($item->user,'statuses_count'))?$item->user->statuses_count:"";

            $sa->postUrl           = 'https://twitter.com/'.$sa->user->profile->displayName."/status/".$sa->id;
    ...
    private function getPostType($post){
        if($post){
            if(!empty($post->retweeted_status))
                return 'retweet';
            if(!empty($post->in_reply_to_status_id_str))
                return 'reply';
            return 'tweet';
        }
    }
